...
public class RechazarSolicitud extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (respuesta.rechazar(dpiSolicitante, cuentaSolicitada)) {
            request.getSession().setAttribute("mensajeRespuesta", "Asociación de cuenta " + cuentaSolicitada + " se rechazó con éxito");
            response.sendRedirect("/Banco/SolicitudDeAsociacionPendiente"); 
        } else {
            request.getSession().setAttribute("mensajeRespuesta", "Error al rechazar la asociación con la cuenta " + cuentaSolicitada + " se realizó con éxito");
            response.sendRedirect("/Banco/SolicitudDeAsociacionPendiente"); 
        }
    }
